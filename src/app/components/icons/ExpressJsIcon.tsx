...
export const ExpressJsIcon = ({ isActive = true, size = 40, style, ...props }: IconProps) => {
  ...
  // ACCESIBILIDAD: Respetar preferencias del sistema
  ...
  const scheduleGlitch = useCallback(() => {
    // Si no animam, no programa el glitch
  ...
  useEffect(() => {
    if (animateIcon && isMounted) scheduleGlitch();
    return () => { if (glitchTimeoutRef.current) clearTimeout(glitchTimeoutRef.current); };
  }, [animateIcon, isMounted, scheduleGlitch]);
  ...
  return (
    ...
    >
      {/* FLUJO DE DATOS */}
      {isMounted && animateIcon && clientPackets.map((p, index) => (
        <m.div
          key={p.id}
          aria-hidden="true"
          style={{
      ...
      <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <m.svg
          ...
          width="100%" height="100%" viewBox="0 0 24 24" fill="#fff"
          style={{ zIndex: 10, overflow: "visible" }}
          aria-hidden="true"
          focusable="false"
          initial={{ x: 0, skewX: 0, filter: "drop-shadow(0px 0px 0px rgba(0,0,0,0))" }}
          animate={animateIcon && isGlitching ? { 
            x: [0, -4, 4, 0],
            skewX: [0, 15, -15, 0],
            filter: [
        ...
        {/* SOMBRAS DE GLITCH */}
        <AnimatePresence>
          {isMounted && animateIcon && isGlitching && glitchPositions.map((pos, i) => (
            <m.svg
              key={`glitch-${i}`}
        ...
        <AnimatePresence>
          {isMounted && animateIcon && isGlitching && (
            <m.div
              aria-hidden="true"
              style={{
                position: "absolute",
