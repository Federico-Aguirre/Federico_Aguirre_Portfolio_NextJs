...
export const AwsIcon = ({ isActive = true, size = 32, style, ...props }: IconProps) => {
  ...
  // Generador de partículas corregido para evitar Hydration Mismatch
  useEffect(() => {
    if (!isActive) {
      setParticles([]);
      return;
    ...
    const interval = setInterval(() => {
      setParticles((prev) => [
        ...prev.slice(-4), 
        { 
          // Usamos un contador o algo que no dependa de Date.now() puro en el primer render
          id: Math.random(), 
          x: Math.random() * 80 - 40, 
          y: Math.random() * 80 - 40 
        },
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* LOGO AWS - CORRECCIÓN: initial filter */}
        <motion.g
          initial={{ filter: "drop-shadow(0 0 0px #F90)" }}
          animate={event === "burst" ? {
            filter: ["drop-shadow(0 0 0px #F90)", "drop-shadow(0 0 8px #F90)", "drop-shadow(0 0 0px #F90)"],
          } : { filter: "drop-shadow(0 0 0px #F90)" }}
        >
          {/* AWS Text - CORRECCIÓN: initial fill */}
          <motion.path
            ...
            fill="#fff"
            initial={{ fill: "#fff" }}
            animate={event === "burst" ? { fill: ["#fff", "#FFD700", "#fff"] } : { fill: "#fff" }}
            transition={{ duration: 0.3 }}
          />

          {/* Sonrisa - CORRECCIÓN: initial pathLength y opacity */}
          <motion.path
