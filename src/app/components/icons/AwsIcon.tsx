...
export const AwsIcon = ({ isActive = true, size = 32, style, ...props }: IconProps) => {
  const [isMounted, setIsMounted] = useState(false);
  const [event, setEvent] = useState<"none" | "sync" | "burst">("none");
  const [particles, setParticles] = useState<{ id: number; x: number; y: number }[]>([]);
  const timeoutRef = useRef<NodeJS.Timeout | null>(null);
  
  // ACCESIBILIDAD: Detecta si el usuario prefiere reducir las animaciones
  ...
  useEffect(() => {
    // Si no está activo, o si el usuario prefiere no ver animaciones, no generamos partículas
  ...
  const scheduleEvent = useCallback(() => {
    // Si el usuario prefiere no ver animaciones, detenemos los eventos "burst" y "sync"
    if (!isActive || shouldReduceMotion) return;
    
    timeoutRef.current = setTimeout(() => {
      const randomEvent = Math.random() > 0.5 ? "sync" : "burst";
  ...
  }, [isActive, scheduleEvent]); 

  if (!isMounted) return <div style={{ width: size, height: size, ...style }} />;
    ...
    >
      <AnimatePresence>
        {isActive && particles.map((p) => (
          <m.div
            key={p.id}
            // ACCESIBILIDAD: Ocultamos las partículas a los lectores de pantalla
            aria-hidden="true" 
            style={{
              position: "absolute",
              width: "2px",
      ...
      <m.svg
        // ACCESIBILIDAD: Vital. Evita que el lector de pantalla intente leer las formas de dibujo
        aria-hidden="true" 
        focusable="false" // Evita que los usuarios de teclado hagan "tab" dentro del SVG por error en navegadores antiguos
        width="100%"
        height="100%"
        viewBox="0 -10 150 120"
        fill="none"
      ...
      >
        ...
        <AnimatePresence>
          {event === "burst" && (
            <m.circle
              ...
              animate={{ opacity: [0, 0.6, 0], scale: 3 }}
              exit={{ opacity: 0 }}
        ...
        {/* LOGO AWS */}
        <m.g
          initial={{ filter: "drop-shadow(0px 0px 0px rgba(255,153,0,0))" }}
          animate={{
            filter: event === "burst" 
              ? [
                  "drop-shadow(0px 0px 0px rgba(255,153,0,0))", 
                  "drop-shadow(0px 0px 8px rgba(255,153,0,0.8))", 
                  "drop-shadow(0px 0px 0px rgba(255,153,0,0))"
                ]
              : "drop-shadow(0px 0px 0px rgba(255,153,0,0))"
          }}
        ...
        >
          ...
          <m.path
            ...
            fill="#fff"
            initial={{ opacity: 1 }}
            animate={event === "burst" ? { fill: ["#ffffff", "#FFD700", "#ffffff"] } : { fill: "#ffffff" }}
          ...
          <m.path
            ...
            animate={event === "sync" ? { pathLength: [0, 1], opacity: [0.5, 1] } : { pathLength: 1, opacity: 1 }}
          ...
          <m.path
            ...
            animate={event === "sync" ? { scale: [1, 1.4, 1] } : { scale: 1 }}
          />
        </m.g>
      </m.svg>
    </div>
