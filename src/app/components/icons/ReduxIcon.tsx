...
export const ReduxIcon = ({
  ...
  // GENERADOR DE VARIANTES PARA LOS ECOS (PULSACIONES)
  const createEchoVariant = (
    maxScale: number,
    maxOpacity: number,
    delay: number
  ): Variants => ({
  ...
  return (
    ...
    >
      {/* ECO 1 (Expansión primaria) */}
      <m.path
        d={logoPath}
        fill={reduxPurple}
        variants={createEchoVariant(1.8, 0.4, 0)}
        initial="idle"
        animate={isActive ? "active" : "idle"}
        style={{ transformOrigin: "16px 16px" }}
      />

      {/* ECO 2 (Expansión secundaria con delay) */}
      <m.path
        d={logoPath}
        fill={reduxPurple}
        variants={createEchoVariant(1.4, 0.6, 0.5)}
        initial="idle"
        animate={isActive ? "active" : "idle"}
        style={{ transformOrigin: "16px 16px" }}
      />
