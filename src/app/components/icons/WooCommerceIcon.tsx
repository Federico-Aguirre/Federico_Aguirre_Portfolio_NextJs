...
export interface WooCommerceIconProps extends SVGMotionProps<SVGSVGElement> {
  /** Activa la animación de levitación sutil y resplandor de neón */
  isActive?: boolean;
  ...
  announcement?: string;
  /** Color dinámico de respaldo si no se usa el gradiente predeterminado */
  color?: string;
}
...
export const WooCommerceIcon = ({
  ...
  role,
  ...props
}: WooCommerceIconProps) => {
  // 1. GENERACIÓN DE ID ÚNICO PARA PREVENIR COLISIONES DE GRADIENTE
  ...
  const iconVariants: Variants = {
    idle: {
      ...
      scale: 1,
      fillOpacity: 1,
      filter: "drop-shadow(0px 2px 4px rgba(112, 0, 255, 0.15))",
      transition: { duration: 0.4, ease: "easeInOut" },
    },
    active: {
      y: shouldReduceMotion ? 0 : -4, // Elevación moderada
      scale: shouldReduceMotion ? 1 : 1.05,
      fillOpacity: 1, // Mantiene contraste
      filter: shouldReduceMotion
        ? "none"
        : "drop-shadow(0px 4px 12px rgba(255, 0, 153, 0.6))",
      transition: {
        y: {
          duration: 2,
          repeat: Infinity,
