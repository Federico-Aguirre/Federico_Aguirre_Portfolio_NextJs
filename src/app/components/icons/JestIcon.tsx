...
export const JestIcon = ({ isActive = false, size = 40, style, ...props }: IconProps) => {
  ...
  const brandRed = "rgba(198, 61, 20, 1)";
  const brandRedGlow = "rgba(198, 61, 20, 0.5)";
  ...
  const bellCoordinates = [
    { cx: 6.5, cy: 15.8, delay: 0 },
    { cx: 16.0, cy: 13.8, delay: 0.6 },
    { cx: 25.5, cy: 15.8, delay: 1.2 }
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        <m.g style={{ transformOrigin: "16px 16px" }}>
          <m.path
            d={backgroundPath}
            fill={brandRed}
          ...
          <m.path
            ...
            fill="#FFFFFF"
            initial={{ opacity: 1 }}
            animate={{ opacity: animateIcon ? 0.2 : 1 }}
            transition={{ duration: 0.4 }}
          />
          ...
          {/* CAMPANAS */}
          <g>
            {isMounted && animateIcon && bellCoordinates.map((bell, i) => (
