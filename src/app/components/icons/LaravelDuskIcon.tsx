...
export const LaravelDuskIcon = ({ isActive = false, size = 40, style, ...props }: IconProps) => {
  ...
  const renderFogChunk = (chunkIndex: number) => (
    <div style={{ position: "relative", width: FOG_CHUNK_WIDTH, height: "100%", flexShrink: 0 }}>
      {fogClouds.map((cloud) => (
        <m.div
          ...
          animate={{ 
            y: animateIcon ? [-10, 10, -10] : 0,
            opacity: cloud.opacity 
          }}
          transition={animateIcon ? {
  ...
  return (
    <div
      ...
      {...props}
    >
      {/* DEFS - Oculto para lectores de pantalla */}
      <svg 
      ...
      >
        <defs>
          ...
          <filter id={`dusk-fog-noise-${uniqueId}`}>
      ...
      >
        ...
        <m.path
          ...
          animate={animateIcon 
            ...
            : { opacity: isActive ? 1 : 0, pathLength: 1, pathOffset: 0 }
          }
          transition={animateIcon ? { duration: 3.5, repeat: Infinity, ease: "easeInOut" } : { duration: 0.4 }}
        />
      </m.svg>
