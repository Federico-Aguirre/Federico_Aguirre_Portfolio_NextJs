...
export const FramerMotionIcon = ({ isActive = true, size = 35, style, ...props }: IconProps) => {
  ...
  // ACCESIBILIDAD: Detectar preferencia del sistema para movimiento reducido
  const shouldReduceMotion = useReducedMotion();
  const animateIcon = isActive && !shouldReduceMotion;

  // Evitar Hydration Mismatch: Generar partículas solo en el cliente
  useEffect(() => {
    setIsMounted(true);
  ...
  return (
    ...
    >
      ...
      {isMounted && animateIcon && clientParticles.map((p) => (
        <m.div
          key={`${uniqueId}-p-${p.id}`}
          // ACCESIBILIDAD: Ocultar elemento puramente decorativo
          aria-hidden="true"
          style={{
            position: "absolute",
            width: p.size,
      ...
      <m.svg
        ...
        animate={{ 
          rotateY: animateIcon ? [0, 15, -15, 0] : 0,
          y: animateIcon ? [0, -2, 0] : 0,
          filter: animateIcon 
      ...
      >
        <m.path
          ...
          animate={{ y: animateIcon ? [-0.5, -2, -0.5] : 0 }}
        ...
        <m.path 
          ...
          animate={{ x: animateIcon ? [0, 3, 0] : 0, y: animateIcon ? [0, 3, 0] : 0 }}
        ...
        <m.path
          ...
          animate={{ 
            scale: animateIcon ? [1, 0.92, 1] : 1, 
            x: animateIcon ? [0, -2, 0] : 0 
          }}
