...
export const GoogleCloudIcon = ({ isActive = true, size = 30, style, ...props }: IconProps) => {
  // Configuración de la animación compartida para crear la "ola"
  ...
  return (
    ...
    >
      ...
      >
        <g>
          {/* NODO ROJO (Arriba) - Empuja hacia arriba */}
          <motion.path
            ...
            animate={isActive ? { opacity: [0.5, 1, 0.5], y: [0, -5, 0] } : { opacity: 1, y: 0 }}
            transition={waveTransition(0)} // Empieza de inmediato
          />
          
          {/* NODO AZUL (Derecha) - Empuja hacia la derecha */}
          <motion.path
            ...
            animate={isActive ? { opacity: [0.5, 1, 0.5], x: [0, 5, 0] } : { opacity: 1, x: 0 }}
            transition={waveTransition(0.5)} // 0.5 segundos después
          />
          
          {/* NODO VERDE (Abajo) - Empuja hacia abajo */}
          <motion.path
            ...
            initial={{ opacity: 1, y: 0 }}
            animate={isActive ? { opacity: [0.5, 1, 0.5], y: [0, 5, 0] } : { opacity: 1, y: 0 }}
            transition={waveTransition(1.0)} // 1.0 segundos después
          />
          
          {/* NODO AMARILLO (Izquierda) - Empuja hacia la izquierda */}
          <motion.path
            d="M71.8704,61.4255 C31.9394,61.6635 -0.2366,94.2275 0.0014,134.1575 C0.1344,156.4555 10.5484,177.4455 28.2234,191.0385 L60.4714,158.7915 C46.4804,152.4705 40.2634,136.0055 46.5844,122.0155 C52.9044,108.0255 69.3704,101.8085 83.3594,108.1285 C89.5244,110.9135 94.4614,115.8515 97.2464,122.0155 L129.4944,89.7685 C115.7734,71.8315 94.4534,61.3445 71.8704,61.4255"
            fill="#FBBC05"
            initial={{ opacity: 1, x: 0 }}
            animate={isActive ? { opacity: [0.5, 1, 0.5], x: [0, -5, 0] } : { opacity: 1, x: 0 }}
            transition={waveTransition(1.5)} // 1.5 segundos después
          />
        </g>
      </motion.svg>
    </div>
