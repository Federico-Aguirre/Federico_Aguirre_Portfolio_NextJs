...
const RouletteWheel = ({ items, selectedId, onSelect, size, gap, renderCard, ariaLabel = "Ruleta de opciones" }: any) => {
  ...
  const tripleItems = useMemo(() => 
    [...items, ...items, ...items].map((it, i) => {
      // Solo el set central será accesible por teclado para no tabular 3 veces lo mismo
      const isInteractive = i >= items.length && i < items.length * 2;
      return { ...it, uKey: `w-${i}-${it.id}`, isInteractive };
    }), 
  [items]);

  const centerWheel = useCallback((isImmediate = false) => {
  ...
  }, [items, selectedId, stride, size, x]);
    ...
    >
      <div style={{ position: 'absolute', top: '50%', left: 0, right: 0, transform: 'translateY(-50%)', display: 'flex', justifyContent: 'space-between', padding: '0 2px', zIndex: 100, pointerEvents: 'none' }}>
        ...
        <m.button 
          ...
          whileHover={{ backgroundColor: "#00E5FF", color: "#000000", scale: 1.1 }}
          transition={{ duration: 0.2 }}
          style={{ 
            pointerEvents: 'auto', background: 'rgba(0,0,0,0.8)', border: '1px solid #00E5FF', color: '#00E5FF', 
            borderRadius: '50%', width: '35px', height: '35px', cursor: 'pointer', fontSize: '20px',
      ...
      <m.div 
        ...
        style={{ x, display: 'flex', gap: `${gap}px`, width: 'max-content', height: '100%', alignItems: 'center' }}
      >
        {tripleItems.map((item) => (
          <div 
            key={item.uKey} 
            role="tab" // Le digo al lector que esto funciona como una pestaña
            aria-selected={item.id === selectedId}
            aria-label={item.name || item.label} // Lee el nombre de la tecnología/categoría
            tabIndex={item.isInteractive ? 0 : -1} // Solo el bloque del medio atrapa el foco (Tab)
            onClick={() => onSelect(item)} 
            onKeyDown={(e) => {
              // Permite accionar la tarjeta con teclado (Espacio o Enter)
            ...
            style={{ 
              ...
              cursor: 'pointer',
              outlineOffset: '4px' // Para que se vea bien el borde al hacer Tab
            }}
          >
  ...
  return (
    <div style={{ 
      ...
      <div style={{ width: '100%', display: 'flex', justifyContent: 'center', padding: '0 5px' }}>
        ...
        >
          <span style={{ color: '#00E5FF', margin: '0 0 5px 0', fontSize: '1rem' }}>{activeTech.label}</span>
